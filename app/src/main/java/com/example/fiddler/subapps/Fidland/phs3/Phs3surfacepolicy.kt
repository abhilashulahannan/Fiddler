 ...
 *
 * Every qualified handler gets an identical 10s rotation turn by default.
 * [SurfacePolicy] makes any deviation from that explicit and per-handler
 * instead of special-cased in `Phs3Manager`.
 *
 * - [CONTINUOUS] — default. Call, Navigation, Music, Timer, Idle, etc.
 *   Behaves exactly like today's round-robin: takes a flat turn in
 *   rotation. Per §B8 #5's Continuous-Dominant resolution, Idle is a plain
 *   CONTINUOUS member like everything else — its old hardcoded carve-out
 *   in `Phs3Manager` is retired. Idle's occasional Special-Condition
 *   promotion is a normal [Phs3RotationPartitioner.interrupt]/
 *   [Phs3RotationPartitioner.resumeAfterEventDriven] call from its own
 *   trigger, same mechanism EVENT_DRIVEN entities use, without needing the
 *   CONTINUOUS/EVENT_DRIVEN distinction itself to change.
 ...
 * - [AMBIENT] — Weather, Comms, wifi band. NOT implemented by
 *   [Phs3RotationPartitioner] yet — value exists so handlers can declare
 *   intent, but per §B9 Phase 1 this is deferred until an entity actually
 *   claims it. No longer blocked on Idle specifically, since Idle resolved
 *   to Continuous-Dominant rather than Ambient (§B8 #5).
 */
