 ...
 * • **Blocks (3) migration — also resolved this pass**, see
 *   [FootballPhs3Handler]'s own class doc: location-a is retired
 *   (`hasLocationA = false`), the Indicator is redeclared
 *   [com.example.fiddler.subapps.Fidland.phs3.BlockAffinity.DYNAMIC], and
 *   the goal/card Lottie + event text now live in a right-anchored
 *   `SecondaryIndicator`, fed by [FootballRepository.flashEventFlow]'s
 *   widened moment coverage (events **and** status transitions).
 *
 ...
 *   }
 *
 * ── Blocks wiring (Phase 4 — location-a retired) ──────────────────────────────
 * [FootballPhs3Handler] no longer opts into location-a (`hasLocationA = false`
 * as of this pass) — the event flash moved into `SecondaryIndicator`, a
 * right-anchored `hasSecondaryBlock`, alongside the primary DYNAMIC
 * Indicator. overlay_fidland_pill wires both automatically via
 * `Phs3BlockPlacementEngine` — no manual cast needed. See
 * FootballPhs3Handler.kt's own class doc for the full block migration.
 *
