...
/**
 * Emitted into [FootballRepository.flashEventFlow] when a NEW
 * [FootballSpecialMoment] is detected — a player-attributed [MatchEvent]
 * (goal/card/sub) or a [MatchStatusTransition] (kickoff/half-time/etc).
 * [FootballPhs3Handler.SecondaryIndicator] shows it for [FLASH_DURATION_MS]
 * then hides it automatically.
 *
 * §B7 Phase 4 — widened from the pre-build version, which only ever carried
 * an [EventType] (goal/card, 4 variants) — the doc's own flag that "trigger
 * surface needs to widen to match the full Special-Condition list". Status
 * transitions have no player to attribute, so [eventType] is null for them
 * and the doc's own resolution applies: render [label] as a plain status
 * phrase with no Lottie icon, rather than inventing one.
 *
 * @param label     Display text — "Goal – Messi" for a player-attributed
 *                  event, "Kick off"/"Full Time"/etc for a status
 *                  transition (see [FootballSpecialMoment.label]).
 * @param eventType The underlying [EventType] when this came from a
 *                  [MatchEvent] (drives [FootballEventIcon]'s asset/colour
 *                  choice) — null when it came from a [MatchStatusTransition],
 *                  which has no matching icon.
 */
...
/** Converts a moment into the flash-display shape — see [FlashEvent]'s doc. */
fun FootballSpecialMoment.toFlashEvent(): FlashEvent = FlashEvent(
    label     = label(),
    eventType = (this as? FootballSpecialMoment.Event)?.event?.type,
)
